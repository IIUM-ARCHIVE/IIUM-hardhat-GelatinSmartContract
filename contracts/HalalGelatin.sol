...
contract HalalGelatinSupplyChain {
    ...
    constructor() {
        admin = msg.sender; // Deployer is the Admin [cite: 39]
    }

    // --- ROLE MANAGEMENT [cite: 34] ---
    // In Remix, use the Deployer account to assign these roles to other accounts (Member 2, Member 3)
    ...
    // 2. Set Halal Certificate (JAKIM verifies the slaughter) [cite: 45]
    ...
    // 3. Transfer Batch (Farm sells to Factory) [cite: 47]
    function transferBatch(string memory _batchId, address _to) external onlyCurrentOwner(_batchId) {
        require(_to != address(0), "Invalid address");
        
        // Validation: Ensure next receiver has the correct role
        // If current owner is Producer, next must be Distributor (Factory)
        if (producers[msg.sender]) {
            require(distributors[_to], "Producer must transfer to a Distributor (Factory)");
        } 
        // If current owner is Distributor, next must be Retailer (Candy Maker)
    ...
    // 4. Update Status (Factory processes bones into Gelatin) [cite: 46]
    // We added _newProductName so you can change "Bones" to "Gelatin"
    function updateStatus(string memory _batchId, string memory _newStatus, string memory _newProductName) external onlyCurrentOwner(_batchId) {
        Batch storage b = batches[_batchId];
        b.status = _newStatus;
        
        // If the product transforms (Istihalah), update the name
